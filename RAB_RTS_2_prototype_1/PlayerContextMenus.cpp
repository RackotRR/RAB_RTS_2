...
PlayerContextMenus::PlayerContextMenus(const SelectedObject& selectedObject, World* world, Player* player)
	:
	world(world),
...
void PlayerContextMenus::Update() {
	...
	for (auto& iter : contextMenus) {
		if (iter.second->GetContextObject() != selectedObject.GetPosition() ||
			selectedObject.IsSmthSelected() == false) {
