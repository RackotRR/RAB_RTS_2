...
World* GameFIO::Load(string path) {
	...
	World* world = new World; // выдел€ет пам€ть дл€ мира (освобождаетс€ где-то в другом месте)
	...
	// ground
	{ 
		level.ground = new Cell * [level.height]; // выдел€етс€ пам€ть под двумерный массив клеток
		...
		for (int j = 0; j < level.height; j++)
			for (int i = 0; i < level.width; i++) {
				int a = 0;
				stream >> a; 
				level.ground[j][i].SetType(static_cast<GroundType>(a)); // дл€ каждой клетки устанавливаетс€ еЄ тип
			}
	};
